...
async function syncIssues(
  ...
  const wanted = new Map<
    string,
    { itemId: string | null; locationId: string | null; code: string; severity: string; detail: unknown }
  >();
  ...
  for (const { scope, output } of results) {
    for (const code of output.reasons) {
      ...
      // This is a site data-health fact, not N copies of an item problem. The
      // engine may still carry the caveat on each result, but the work queue
      // gets one site-wide row so the UI does not shout the same thing forever.
      if (code === 'UNMATCHED_MOVEMENTS_AT_SITE') {
        const key = `null::none::${code}`;
        if (!wanted.has(key)) {
          wanted.set(key, {
            itemId: null,
            locationId: null,
