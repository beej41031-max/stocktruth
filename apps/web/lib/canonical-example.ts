...
import { DEFAULT_POLICY, explain, asOfKnowledge, reconcile, assessOperationalPosition, type Explanation, type OperationalPosition } from '@stocktruth/engine';
...
export async function loadCanonicalExample(
  ...
  const now = explain(reconciliationInput);
  const rawResult = reconcile(reconciliationInput);
  ...
  const unboundedReasons = new Set(['MOVEMENT_UNDATED', 'MOVEMENT_MAY_BELONG_HERE', 'ITEM_BLOCKED', 'AMBIGUOUS_ITEM_IDENTITY', 'MOVEMENT_UNIT_MISMATCH', 'COUNT_UNIT_MISMATCH']);
  const operational = assessOperationalPosition({
    result: rawResult,
    spanningMovements: spanning ? [spanning] : [],
    hasUnboundedBlocker: rawResult.reasons.some((r) => unboundedReasons.has(r)),
  });
