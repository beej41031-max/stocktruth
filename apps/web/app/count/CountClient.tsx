    ...
    <>
      ...
      {saved && (
        <div style={{ marginTop: 26, maxWidth: 460 }}>
          <h2 style={{ marginTop: 0 }}>{saved.message}</h2>
          <p className="sub">
            {saved.item} · you counted {saved.quantity} {saved.unit}
          </p>

          {saved.reveal?.book ? (
            <>
              <div className="truth" style={{ gridTemplateColumns: '1fr 1fr 1fr' }}>
                <div>
                  <div className="label">You counted</div>
                  <div className="value">{saved.quantity}</div>
                </div>
                <div>
                  <div className="label">Records said</div>
                  <div className="value">{saved.reveal.book.quantity}</div>
                  <div className="when">
                    {saved.reveal.book.asOf
                      ? `as at ${new Date(saved.reveal.book.asOf).toLocaleDateString('en-GB')}`
                      : 'no date given'}
                  </div>
                </div>
                <div>
                  ...
                  <div className="value">
                    ...
                    {saved.reveal.difference}
                  </div>
                </div>
              </div>
              {saved.reveal.difference !== 0 && (
                <p className="sub">
                  Those two disagree. Whether that means anything depends on what moved between
                  the book date and now, which the reconciliation works out. No adjustment has
                  been made.
                </p>
              )}
            </>
          ) : (
            <p className="sub">Nothing on record to compare it against. The count stands on its own.</p>
          )}
        </div>
      )}
