...
export default function CountClient({ siteId, siteName, locations, openSession, recent }: Props) {
  ...
  return (
    <>
      ...
      <div className="banner" style={{ marginTop: 32, maxWidth: 720 }}>
        <strong>Finish this count session</strong>
        <p className="sub" style={{ marginTop: 8 }}>
          This closes only the physical count. It does not certify that receipts, transfers or production paperwork are complete.
          An owner or manager confirms those records later, against this count cutoff, after the office systems have caught up.
        </p>
        <div style={{ marginTop: 14 }}>
          <button
            onClick={() =>
              start(async () => {
                const res = await completeSession({ siteId, sessionId });
                setCloseMessage(res.message);
                if (res.ok) setSessionId(null);
              })
            }
            disabled={pending}
          >
            {pending ? 'Closing' : 'Close physical count'}
          </button>
        </div>
        {closeMessage && <p className="sub" style={{ marginTop: 10 }}>{closeMessage}</p>}
      </div>
