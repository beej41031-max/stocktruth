...
export async function saveCount(payload: SaveCountInput): Promise<SaveCountResult> {
  ...
  return withUser(userId, async (db) => {
    const session = await db.one<{ id: string }>(
      `select id from count_sessions where id = $1 and site_id = $2 and status = 'open'`,
      [payload.sessionId, payload.siteId],
    );
    if (!session) {
      return { status: 'refused', message: 'This count session is closed or unavailable. Start a new session.' };
    }
...
/**
 * Close the physical counting activity only. Evidence completeness is a
 * separate office/owner assertion made later, after paperwork and automated
 * feeds have had time to catch up. The counter must never be asked to certify
 * information they cannot see from the rack.
 */
export async function completeSession(input: CompleteSessionInput): Promise<{ ok: boolean; message: string }> {
  const userId = await currentUserId();
  return withUser(userId, async (db) => {
    const session = await db.one<{ id: string; organisation_id: string; cutoff_at: string | null; received_cutoff_at: string | null }>(
      `select cs.id, cs.organisation_id, max(cl.counted_at)::text as cutoff_at, max(cl.received_at)::text as received_cutoff_at
         from count_sessions cs
         left join count_lines cl on cl.count_session_id = cs.id and not cl.superseded
        where cs.id = $1 and cs.site_id = $2 and cs.status = 'open'
        group by cs.id, cs.organisation_id`,
      [input.sessionId, input.siteId],
    );
    if (!session) return { ok: false, message: 'Open count session not found.' };
    if (!session.cutoff_at) return { ok: false, message: 'There are no saved counts in this session.' };
    ...
    await db.query(
      `insert into audit_events
       ...
       values ($1,$2,$3,'user','COUNT_SESSION_CLOSED','count_session',$4,$5)`,
      [
        session.organisation_id,
        input.siteId,
        userId,
        input.sessionId,
        JSON.stringify({ physicalCutoffAt: session.cutoff_at, trustedEvidenceCutoffAt: session.received_cutoff_at, evidenceAttested: false }),
      ],
    );
    ...
    return {
      ok: true,
      message: 'Count session closed. Evidence is not certified yet; office/owner confirmation happens later after records catch up.',
    };
  });
}
