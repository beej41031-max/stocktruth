...
test('two identical receipts minutes apart block an exact current position', () => {
  const t = day(1);
  const r = reconcile(
    input({
      movements: [
        move('RECEIVE', 10, { id: 'receipt-a', occurredAt: t, recordedAt: t }),
        move('RECEIVE', 10, { id: 'receipt-b', occurredAt: new Date(t.getTime() + 60_000), recordedAt: t }),
      ],
    }),
  );
  assert.equal(r.state, 'INCOMPLETE');
  assert.equal(r.derivedQuantity, null);
  assert.equal(r.movementNet, null);
  assert.ok(r.reasons.includes('SUSPECTED_DUPLICATE_MOVEMENT'));
  assert.ok(r.evidence.ignoredMovementIds.includes('receipt-a'));
  assert.ok(r.evidence.ignoredMovementIds.includes('receipt-b'));
});

test('duplicate detection is shape-aware even when another movement is interleaved', () => {
  const t = day(1);
  const r = reconcile(
    input({
      movements: [
        move('RECEIVE', 10, { id: 'dup-a', occurredAt: t, recordedAt: t }),
        move('RECEIVE', 25, { id: 'other', occurredAt: new Date(t.getTime() + 60_000), recordedAt: t }),
        move('RECEIVE', 10, { id: 'dup-b', occurredAt: new Date(t.getTime() + 120_000), recordedAt: t }),
      ],
    }),
  );
  assert.equal(r.state, 'INCOMPLETE');
...
test('same-shape receipts outside the duplicate window remain independent', () => {
  const t = day(1);
  const r = reconcile(
    input({
      movements: [
        move('RECEIVE', 10, { id: 'receipt-a', occurredAt: t, recordedAt: t }),
        move('RECEIVE', 10, { id: 'receipt-b', occurredAt: new Date(t.getTime() + 6 * 60_000), recordedAt: t }),
      ],
    }),
  );
  assert.ok(!r.reasons.includes('SUSPECTED_DUPLICATE_MOVEMENT'));
  assert.equal(r.derivedQuantity, 58);
});

test('duplicate-looking movements wholly before the physical count do not poison the anchored current position', () => {
  const t = day(5);
  const r = reconcile(
    input({
      book: null,
      movements: [
        move('RECEIVE', 10, { id: 'old-a', occurredAt: t, recordedAt: t }),
        move('RECEIVE', 10, { id: 'old-b', occurredAt: new Date(t.getTime() + 60_000), recordedAt: t }),
      ],
    }),
  );
  assert.ok(!r.reasons.includes('SUSPECTED_DUPLICATE_MOVEMENT'));
  assert.equal(r.derivedQuantity, 38, 'the later physical count absorbs pre-count ledger ambiguity');
});
...
// ---------------------------------------------------------------------------
// Input-boundary trust regressions (v0.4.5)
// ---------------------------------------------------------------------------

test('a negative movement quantity is invalid evidence, not inverted direction', () => {
  const bad = move('RECEIVE', -5);
  const r = reconcile(input({ count: count({ quantity: 20 }), movements: [bad] }));
  assert.equal(r.state, 'CONFLICT');
  assert.equal(r.derivedQuantity, null);
  assert.ok(r.reasons.includes('MOVEMENT_QUANTITY_INVALID'));
  assert.ok(r.evidence.ignoredMovementIds.includes(bad.id));
});

test('a bare adjustment after the count blocks the current position instead of assuming plus', () => {
  const adjustment = move('ADJUST', 3);
  const r = reconcile(input({ count: count({ quantity: 20 }), movements: [adjustment] }));
  assert.equal(r.state, 'INCOMPLETE');
  assert.equal(r.derivedQuantity, null);
  assert.equal(r.movementNet, 0, 'known directed movement net remains visible without applying the adjustment');
  assert.ok(r.reasons.includes('ADJUSTMENT_IN_INTERVAL'));
  assert.ok(r.evidence.ignoredMovementIds.includes(adjustment.id));
});

test('duplicate-looking pre-count movements do not alter current stock but do invalidate book-to-count variance', () => {
  const t = day(5);
  const r = reconcile(
    input({
      movements: [
        move('RECEIVE', 10, { id: 'bookdup-a', occurredAt: t, recordedAt: t }),
        move('RECEIVE', 10, { id: 'bookdup-b', occurredAt: new Date(t.getTime() + 60_000), recordedAt: t }),
      ],
    }),
  );

  assert.equal(r.derivedQuantity, 38, 'the physical count still anchors the current position');
  assert.equal(r.varianceAtCount, null, 'ledger drift cannot be measured through duplicate movement evidence');
  assert.ok(r.reasons.includes('BOOK_WINDOW_DUPLICATE_MOVEMENT'));
  assert.ok(!r.reasons.includes('SUSPECTED_DUPLICATE_MOVEMENT'));
  assert.equal(r.state, 'PROVISIONAL');
});
