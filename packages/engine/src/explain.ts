...
function evidenceFor(
  ...
  switch (code) {
    ...
    case 'ADJUSTMENT_IN_INTERVAL':
      return count
        ? movements
            .filter((m) => m.type === 'ADJUST' && m.occurredAt != null && m.occurredAt.getTime() > count.countedAt.getTime())
            .map((m) => `movement ${m.id}: adjustment of ${m.quantity} at ${m.occurredAt!.toISOString()}`)
        : [];
    case 'BOOK_GAP_UNATTRIBUTABLE':
      return book && count
        ? [
            `count ${count.id}: ${count.quantity} at ${count.countedAt.toISOString()}`,
            `book ${book.id}: ${book.quantity} at ${book.asOf?.toISOString()}`,
            `gap after visible movements: ${result.varianceAtCount}`,
          ]
        : [];
    default:
      return [];
...
function ref(prefix: string, id: string): string {
  ...
  // Path-style ids (Shopify's gid://shopify/Thing/123) use the last segment.
  const tail = id.includes('-') ? id.split('-').pop()! : id.includes('/') ? id.split('/').pop()! : id;
...
function resolutionFor(
  ...
  switch (code) {
    ...
    case 'ADJUSTMENT_IN_INTERVAL': {
      const adjustments = count
        ? movements.filter(
            (m) => m.type === 'ADJUST' && m.occurredAt != null && m.occurredAt.getTime() > count.countedAt.getTime(),
          )
        : [];
      const list = adjustments
        .map((m) => `${ref('M', m.id)} (${m.quantity} ${unit}, ${m.occurredAt!.toISOString()})`)
        .join(', ');
      return (
        `${adjustments.length === 1 ? 'Adjustment' : 'Adjustments'} ${list} since the count ` +
        `${adjustments.length === 1 ? 'does' : 'do'} not say whether stock went in or out. Find out what ` +
        `${adjustments.length === 1 ? 'it' : 'they'} recorded and replace ${adjustments.length === 1 ? 'it' : 'them'} ` +
        `with an explicit receipt or issue, or count again.`
      );
    }

    case 'BOOK_GAP_UNATTRIBUTABLE': {
      const gap = result.varianceAtCount ?? 0;
      const expected = book ? book.quantity + gap : null;
      return (
        `The count and every movement the feed can see since it add up to ${expected} ${unit}; ` +
        `the system says ${book?.quantity}. That ${Math.abs(gap)} ${unit} ` +
        `${gap < 0 ? 'more in the system than the evidence supports' : 'less in the system than the evidence supports'} ` +
        `could be real ${gap < 0 ? 'loss' : 'unrecorded stock'}, or a change the feed cannot show, such as a manual ` +
        `adjustment or a receipt entered by hand. Check for those since ${count?.countedAt.toISOString()} before ` +
