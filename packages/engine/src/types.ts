...
export const MOVEMENT_SIGN: Record<MovementType, 1 | -1> = {
  ...
  TRANSFER_OUT: -1,
  WASTE: -1,
  // An adjustment carries its own direction in the quantity's sign upstream;
  // by the time it reaches here it has been normalised to a positive number
  // plus a type, so treat it as an increase and let the importer emit a
  // separate negative movement where it means a decrease.
  ADJUST: 1,
};
