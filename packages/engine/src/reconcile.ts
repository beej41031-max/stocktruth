...
export function reconcile(input: ReconciliationInput): ReconciliationOutput {
  ...
  // Quantity sign is not direction. A negative RECEIVE must never become an
  // issue merely because arithmetic happens to work. Adapters should reject
  // these rows; the engine repeats the check because adapter trust is not a
  // safety boundary.
  const invalidQuantityMovements = movements.filter(
    (m) => !Number.isFinite(m.quantity) || m.quantity < 0,
  );
  if (invalidQuantityMovements.length > 0) {
    reasons.add('MOVEMENT_QUANTITY_INVALID');
    for (const m of invalidQuantityMovements) ignoredMovementIds.push(m.id);
    return out('CONFLICT');
  }
  ...
  // Duplicate evidence is evaluated after reversal normalisation. A valid
  // correction pair must never look like a duplicate, while a duplicate that
  // touches the post-count window makes the current position non-exact.
  const duplicateNormalised = removeReversalPairs(dated);
  const duplicateGroups = findSuspectedDuplicateMovementGroups(duplicateNormalised.effective);
  const duplicateCurrentIds = new Set(
    duplicateGroups
      .filter((group) => group.movements.some((m) => m.occurredAt.getTime() > count.countedAt.getTime()))
      .flatMap((group) => group.movementIds),
  );
  const duplicateAffectsCurrent = duplicateCurrentIds.size > 0;
  if (duplicateAffectsCurrent) reasons.add('SUSPECTED_DUPLICATE_MOVEMENT');

  // Movements strictly after the count are the ones we apply. A source-level
  // ADJUST is not applied because its direction is not explicit. Treating it
  // as +1 would turn an accounting label into physical fact.
  const adjustmentAfterCount = dated.filter(
    (m) => m.occurredAt.getTime() > count.countedAt.getTime() && m.type === 'ADJUST',
  );
  if (adjustmentAfterCount.length > 0) {
    reasons.add('ADJUSTMENT_IN_INTERVAL');
    for (const m of adjustmentAfterCount) ignoredMovementIds.push(m.id);
  }

  const afterCount = dated
    .filter((m) => m.occurredAt.getTime() > count.countedAt.getTime() && m.type !== 'ADJUST')
    .sort((a, b) => a.occurredAt.getTime() - b.occurredAt.getTime());

  const netAfter = afterCount.reduce((sum, m) => sum + MOVEMENT_SIGN[m.type] * m.quantity, 0);
  for (const m of afterCount) {
    if (duplicateCurrentIds.has(m.id)) ignoredMovementIds.push(m.id);
  ...
  if (!movementFeedComplete) reasons.add('MOVEMENT_FEED_PARTIAL');

  if (!book) {
    reasons.add('NO_BOOK_POSITION');
  } else if (book.asOf == null) {
    reasons.add('BOOK_UNDATED');
  } else if (book.asOf.getTime() > count.countedAt.getTime()) {
    // The book is newer than the count: normal for any live system, where the
    // platform's figure is "now" and the count was this morning. Comparing the
    // two directly would call every sale in between a loss. Instead the count
    // is carried forward to the book's moment, and the two compared there.
    // Decision 0025.
    ...
    // A movement that happened before the book's moment but was written down
    // after it may or may not be inside the book's figure. Same problem as a
    // movement spanning the count, one clock later.
    const spansBook = dated.some(
      (m) =>
        m.occurredAt.getTime() <= book.asOf!.getTime() &&
        m.recordedAt != null &&
        m.recordedAt.getTime() > book.asOf!.getTime(),
    );
    const duplicateAffectsBookWindow = duplicateGroups.some((group) =>
    ...
    if (duplicateAffectsBookWindow) reasons.add('BOOK_WINDOW_DUPLICATE_MOVEMENT');
    ...
    const windowTrustworthy =
      ...
      !duplicateAffectsBookWindow &&
      !between.some((m) => m.type === 'ADJUST');
    ...
    if (windowTrustworthy) {
      ...
      if (varianceAtCount !== 0 && !movementFeedComplete) {
        ...
        // reported, but neither side of it can be called right.
        reasons.add('BOOK_GAP_UNATTRIBUTABLE');
      }
    }
  } else {
  ...
  // -------------------------------------------------------------------------
  // 8. Duplicate evidence is uncertainty, not arithmetic. If the suspected
  //    duplicate touches the post-count window there is no single defensible
  //    movement net, so refuse before using the duplicated arithmetic to infer
  //    a contradiction such as a negative position.
  // -------------------------------------------------------------------------

  if (duplicateAffectsCurrent) {
  ...
  // -------------------------------------------------------------------------
  // 9. Contradiction beats everything below it.
  // -------------------------------------------------------------------------
  ...
  // -------------------------------------------------------------------------
  // 10. State. Blocking problems first, then age, then caveats.
  // -------------------------------------------------------------------------
